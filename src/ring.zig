...
//! `[u16 len][u8 flags][bytes]`; evicts oldest records when full.
//! Pure code — unit-tests run on any host.
...
const header_len = 3; // u16 len (little endian) + u8 flags
...
pub fn Ring(comptime capacity: usize) type {
    return struct {
        ...
        /// are rejected.
        pub fn push(self: *Self, line: []const u8, flags: u8) bool {
        ...
        pub const Record = struct { line: []const u8, flags: u8 };
        ...
        pub const Iterator = struct {
            ...
            pub fn next(self: *Iterator) ?Record {
                ...
                return .{ .line = self.copy_buf[0..len], .flags = flags };
            }
        };
...
const T = Ring(64); // tiny ring so eviction is easy to exercise
var test_copy: [4096]u8 = undefined;

test "push then iterate FIFO with flags" {
    var r: T = .{};
    try std.testing.expect(r.push("hello", 0));
    try std.testing.expect(r.push("world", flag_stderr));
...
test "eviction keeps newest records" {
    var r: T = .{};
    // 64-byte ring, each record costs 3 + len
    try std.testing.expect(r.push("aaaaaaaaaa", 0)); // 13
    try std.testing.expect(r.push("bbbbbbbbbb", 0)); // 26
    try std.testing.expect(r.push("cccccccccc", 0)); // 39
    try std.testing.expect(r.push("dddddddddd", 0)); // 52
    try std.testing.expect(r.push("eeeeeeeeee", 0)); // would be 65 -> evict "a"
    var it = r.iterate(&test_copy);
    const first = it.next().?;
...
test "record larger than ring is rejected" {
    var r: T = .{};
    const big = "x" ** 100;
    try std.testing.expect(!r.push(big, 0));
    try std.testing.expectEqual(@as(usize, 0), r.count());
}

test "wrap-around record comes back contiguous" {
    var r: T = .{};
    try std.testing.expect(r.push("0123456789012345678901234567890123456789", 0)); // 43 bytes used
    try std.testing.expect(r.push("abcdefghijklmnopqrstuvwxyz", 0)); // evicts, wraps
    var it = r.iterate(&test_copy);
    const rec = it.next().?;
